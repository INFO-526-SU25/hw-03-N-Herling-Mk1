...
explanatory_labels <- c(
  ...
  All = "All respondents"  # Optional: used if you're keeping the "All" group
)

# Filter out "All" if you want to plot only grouped data:
...
response_labels <- c(
  ...
  resp_feel_safe_at_work = "Vaccine makes me feel safer at work",
...
# Clean plot data: ensure explanatory_value is a factor (for consistent order)
covid_survey_summary_stats <- covid_survey_summary_stats %>%
  mutate(
    explanatory_value = fct_inorder(as.character(explanatory_value)),
    response = factor(response, levels = names(response_labels)),
    explanatory = factor(explanatory, levels = names(explanatory_labels))
  )

# Build the plot
covid_subset <- covid_survey_summary_stats %>%
  filter(explanatory %in% c("All", "exp_age_bin")) %>%
  filter(is.finite(mean), is.finite(low), is.finite(high))

g1 <- ggplot(covid_subset, aes(x = explanatory_value, y = mean)) +
  ...
  labs(
    title = "COVID-19 Vaccine Survey: Overall vs Age Groups",
    x = "Group",
    y = "Mean Response (1–5 Likert)"
  ) +
  theme(
