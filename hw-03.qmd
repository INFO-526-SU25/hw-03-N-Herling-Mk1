...
# Start from covid_survey_summary_stats_by_group only - Age
covid_age_only <- covid_survey_summary_stats_by_group %>%
  filter(explanatory == "exp_age_bin") %>%
  filter(is.finite(mean), is.finite(low), is.finite(high)) %>%
  mutate(
    # Convert encoded bins to readable labels for age
    ...
    # Update explanatory factor to include 'All', 'exp_gender', and 'exp_race'
    explanatory = factor(explanatory, levels = c("All", "exp_age_bin", "exp_gender", "exp_race"))
  )

# - gender
covid_gender_only <- covid_survey_summary_stats_by_group %>%
...
# ----------- race data
# New: filter for Race layer data
covid_race_only <- covid_survey_summary_stats_by_group %>%
...
# --------
# New: filter for Ethnicity layer data
covid_ethnicity_only <- covid_survey_summary_stats_by_group %>%
  filter(explanatory == "exp_ethnicity") %>%
  filter(is.finite(mean), is.finite(low), is.finite(high)) %>%
  mutate(
    # Map coded ethnicity values to readable labels
    explanatory_value = recode(as.character(explanatory_value),
                               "1" = "Hispanic/Latino",
                               "2" = "Non-Hispanic/Non-Latino"
    ),
    # Set vertical display order: top = Hispanic, bottom = Non-Hispanic
    explanatory_value = factor(explanatory_value, levels = c(
      "Hispanic/Latino",
      "Non-Hispanic/Non-Latino"
    )),
    explanatory = factor(explanatory, levels = c("All", "exp_age_bin", "exp_gender", "exp_ethnicity"))
  )
#---------- check encoding data.
# View distinct codes used in the exp_ethnicity variable
# ethnicity_names <- survey_clean %>%
#   select(exp_ethnicity) %>%
#   distinct() %>%
#   arrange(exp_ethnicity)
#
# print(ethnicity_names)
...
covid_ethnicity_only <- covid_survey_summary_stats_by_group %>%
  ...
  mutate(
    ...
    explanatory = factor(explanatory, levels = c("All", "exp_age_bin", "exp_gender", "exp_race", "exp_ethnicity"))
  )
...
# First layer: column headers and overall stats ("All")
g0 <- ggplot(covid_all_only, aes(x = explanatory_value, y = mean)) +
  ...
  labs(
    title = "COVID-19 Vaccine Survey: Mean Response by Age",
    x = NULL,
    y = NULL  # Removed x-axis label from row_1 here
  ) +
  theme(
...
# Second layer: Age
g1 <- ggplot(covid_age_only, aes(x = explanatory_value, y = mean, group = explanatory_value)) +
  geom_errorbar(aes(ymin = low, ymax = high), width = 0.2) +
  geom_point(size = 3, color = "black") +
  coord_flip() +
  facet_grid(
  ...
  theme(
    ...
    panel.spacing = unit(1, "lines"),
    axis.text.x = element_blank(),     # <-- Removed x-axis text for Age row
    axis.ticks.x = element_blank()     # <-- Removed x-axis ticks for Age row
  )

# Third layer: Gender (new)
g2 <- ggplot(covid_gender_only, aes(x = explanatory_value, y = mean, group = explanatory_value)) +
  geom_errorbar(aes(ymin = low, ymax = high), width = 0.2) +
  geom_point(size = 3, color = "black") +
  coord_flip() +
  facet_grid(
  ...
  theme(
    ...
    strip.text.y.right = element_text(angle = 0, vjust = 0.5),
    axis.text.y = element_text(size = 10),
    axis.text.x = element_blank(),  # <---- here
    panel.spacing = unit(1, "lines")
  )

# Fourth layer: Race (new)
g3 <- ggplot(covid_race_only, aes(x = explanatory_value, y = mean, group = explanatory_value)) +
  geom_errorbar(aes(ymin = low, ymax = high), width = 0.2) +
  geom_point(size = 3, color = "black") +
  coord_flip() +
  facet_grid(
...
g4 <- ggplot(covid_ethnicity_only, aes(x = explanatory_value, y = mean, group = explanatory_value)) +
  geom_errorbar(aes(ymin = low, ymax = high), width = 0.2) +
  geom_point(size = 3, color = "black") +
  coord_flip() +
  facet_grid(
  ...
  labs(
    x = NULL,
    y = "Mean Likert score\n(Error barns range from 10th to 90th percentile"
  ) +
  theme(
    strip.background = element_rect(fill = "gray90", color = "black"),
    strip.placement = "outside",
    strip.text.x = element_blank(),  # remove x-axis labels on top (optional)
    strip.text.y.right = element_text(angle = 0, vjust = 0.5),
    axis.text.y = element_text(size = 10),  # show y-axis labels clearly
    panel.spacing = unit(1, "lines"),
    axis.text.x = element_text(size = 10),  # optional, keep x-axis labels if desired
    axis.ticks.x = element_line()           # show ticks on x-axis
  )
